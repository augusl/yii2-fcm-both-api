...
class TopicSubscribeResponse extends AbstractResponse implements TopicSubscribeResponseInterface
{
    ...
    public function validateResponse($response): bool
    {
        ...
        if (400 === $statusCode) {
            \Yii::info(ErrorsHelper::getStatusCodeErrorMessage($statusCode, $responseContents, $this), ErrorsHelper::GUZZLE_HTTP_CLIENT_ERROR);
            \Yii::info('Something in the request data was wrong: check if all data{...}values are converted to strings.', ErrorsHelper::GUZZLE_HTTP_CLIENT_ERROR);
        ...
        if (401 === $statusCode) {
            \Yii::info(ErrorsHelper::getStatusCodeErrorMessage($statusCode, self::UNAUTHORIZED_REQUEST_EXCEPTION_MESSAGE, $this), ErrorsHelper::GUZZLE_HTTP_CLIENT_ERROR);
            \Yii::info('To use the new FCM HTTP Legacy API, you need to enable FCM API on your Google API dashboard first - https://console.developers.google.com/apis/library/fcm.googleapis.com/.', ErrorsHelper::GUZZLE_HTTP_CLIENT_ERROR);
            $this->setErrorStatusDescription(ErrorsHelper::STATUS_CODE_403, $responseContents);
            return false;
        }

        \Yii::info(ErrorsHelper::getStatusCodeErrorMessage($statusCode, $responseContents, $this), ErrorsHelper::GUZZLE_HTTP_CLIENT_OTHER_ERRORS);
